...
def create_input_fields(root, method, labels, input_values):
    for label_text in labels:
        Label(root, text=label_text, font=("Helvetica", 12, "bold"), background="#e74c3c", foreground="white").pack(pady=5)
        entry = Entry(root, font=("Helvetica", 12), background="#ecf0f1")
        entry.pack(pady=5)
        input_values.append(entry)

    solve_method = lambda: solve_equation(method, [float(entry.get()) for entry in input_values])
    Button(root, text=f"Solve using {method}", command=solve_method, font=("Helvetica", 14, "bold"), background="#3498db", foreground="white").pack(pady=10)
...
def create_gui():
    ...
    # Method selection
    Label(root, text="Pilih Metode", font=("Helvetica", 16, "underline"), background="#3498db", foreground="white").pack(pady=10)
    ...
    method_dropdown.config(font=("Helvetica", 12), background="#2ecc71", foreground="white")
