...
def create_input_fields(root, method, labels, input_values):
    for label_text in labels:
        Label(root, text=label_text).pack(pady=5)
        entry = Entry(root)
        entry.pack(pady=5)
        input_values.append(entry)

    solve_method = lambda: solve_equation(method, [float(entry.get()) for entry in input_values])
    Button(root, text=f"Solve using {method}", command=solve_method).pack(pady=10)
...
def create_gui():
    ...
    # Method selection
    Label(root, text="Pilih Metode").pack(pady=10)
